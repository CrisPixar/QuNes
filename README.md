...
- темы System, Light, Dark и экспериментальная Pink Furry (если включена в сборке)
...
В release запрещены локальные HTTP-профили и debug log viewer; production по умолчанию — `https://mainquantumserver.onrender.com`. В debug beta-тестеру доступен viewer логов. Логи не содержат пароли, токены, приватные ключи или plaintext сообщений. Root admin, verified, beta tester и scam reason проверяются сервером, а не только UI. После logout приложение очищает локальные сообщения и состояние ratchet текущего аккаунта.
