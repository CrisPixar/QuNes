...
public interface MessageDao {
    ...
    @Query("SELECT * FROM messages WHERE id = :id LIMIT 1") Maybe<MessageEntity> getById(String id);
    @Query("SELECT * FROM messages WHERE clientMessageId = :clientId LIMIT 1") Maybe<MessageEntity> getByClientMessageId(String clientId);
    ...
    @Query("UPDATE messages SET decryptedCache = :text, decryptionFailed = 0, decryptionError = NULL WHERE id = :id") Completable setDecrypted(String id, String text);
    @Query("UPDATE messages SET decryptionFailed = 1, decryptionError = :error WHERE id = :id") Completable markFailed(String id, String error);
    @Query("UPDATE messages SET delivered = 1 WHERE id = :id") Completable markDelivered(String id);
    @Query("UPDATE messages SET delivered = 1 WHERE clientMessageId = :clientId") Completable markDeliveredByClientId(String clientId);
    ...
    @Query("DELETE FROM messages") Completable clearAll();
}
